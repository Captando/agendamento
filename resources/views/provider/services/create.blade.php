<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Novo Servico</h2>
    </x-slot>
                ...
                <form method="POST" action="{{ route('provider.services.store') }}">
                    ...
                    <div class="mt-4 grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="duration_minutes" name="duration_minutes" type="number" class="mt-1 block w-full" :value="old('duration_minutes', 30)" min="5" max="480" required />
                            <x-input-error :messages="$errors->get('duration_minutes')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="price" value="Preco (R$)" />
                            <x-text-input id="price" name="price" type="number" step="0.01" class="mt-1 block w-full" :value="old('price')" min="0" required />
                    ...
                    <div class="flex items-center justify-end mt-6">
                        ...
                        <x-primary-button>Criar Servico</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
