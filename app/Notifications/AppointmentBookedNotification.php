...
class AppointmentBookedNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        return (new MailMessage)
            ->subject('Novo Agendamento - ' . $appt->service->name)
            ->greeting('Ola, ' . $notifiable->name . '!')
            ->line('Um novo agendamento foi realizado.')
            ->line('**Servico:** ' . $appt->service->name)
            ->line('**Data:** ' . $appt->date->format('d/m/Y'))
            ->line('**Horario:** ' . $appt->start_time . ' - ' . $appt->end_time)
            ->line('**Valor:** ' . $appt->formattedPrice())
            ->line('**Status:** Pendente')
            ->line('Obrigado por usar nosso sistema de agendamento!');
    }
}
