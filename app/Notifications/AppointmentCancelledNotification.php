...
class AppointmentCancelledNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        return (new MailMessage)
            ->subject('Agendamento Cancelado - ' . $appt->service->name)
            ->greeting('Ola, ' . $notifiable->name . '.')
            ->line('Infelizmente, o agendamento abaixo foi cancelado.')
            ->line('**Servico:** ' . $appt->service->name)
            ->line('**Data:** ' . $appt->date->format('d/m/Y'))
            ->line('**Horario:** ' . $appt->start_time . ' - ' . $appt->end_time)
            ->line('Caso deseje, voce pode realizar um novo agendamento.');
    }
}
